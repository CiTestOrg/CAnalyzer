    ...
    companion object {
        ...
        private fun process(prefix: String, currentNode: JsonNode, localVarSet: HashSet<String>, varsCollector: HashMap<String, String>) {
            if (currentNode.isArray) {
                ...
                var index = 1
                while (node.hasNext()) {
                    process(if (prefix.isNotEmpty()) "$prefix-$index" else index.toString(), node.next(), localVarSet, varsCollector)
                    index += 1
                }
            } else if (currentNode.isObject) {
                val varNode = currentNode.get("variables")
                varNode?.fields()?.forEachRemaining { (key, value): Map.Entry<String, JsonNode> ->
        ...
        fun main(args: Array<String>) {
            ...
            inFile.forEachLine { line ->
                ...
                try {
                    ...
                    val seedSecrets = SecretVarTool.getSecretVars(allUsedVars, varSet)
